...
#VIEW ONLY TRANSACTIONS FROM CITIES STARTING WITH THE LETTER K
SELECT * FROM transactions
WHERE City RLIKE '^[K].*$'
...
#COMPARE THE TOTAL TRANSACTION VOLUMES AND VALUES FOR EACH GENDER
SELECT Gender, COUNT(*),
...
#SEE THE AREAS IN WHICH FEMALES SPEND MONEY ON
SELECT exp_type,
...
#CALCULATE THE TOTAL TRANSACTION VALUES BY EACH YEAR AND MONTH
SELECT YEAR(Date) AS YEAR, MONTH(Date) AS MONTH, SUM(Amount) AS AMOUNT
FROM transactions
GROUP BY YEAR, MONTH
ORDER BY YEAR, MONTH ASC

#THE TOP 3 TRANSACTION AMOUNTS BY EACH EXPENDITURE TYPE AND THE CARD TYPE THE PURCHASE WAS MADE WITH
SELECT *
    FROM (
        SELECT  exp_type,
...
SELECT t.*
    FROM ( SELECT City, MAX(Amount) AS Amount
               FROM transactions
               GROUP BY City ) x
    JOIN transactions t USING(City, Amount)
    ORDER BY City ASC